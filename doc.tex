...
\title{Likelihood ratio test for a gamma distribution}
...
\begin{document}
	...
	\section{Definitions}
	Let $Y_{ij}$ denote the $j$th observation from the $i$th treatment group, $i=1, 2, 3, ..., m$ and $j=1, 2, 3, ..., n_i$. 
	...
	Differentiating our log-likelihood with respect to $\beta_k$ and setting the derivative to zero:
	...
	And we know that asymptotically: $-2\ln{\lambda} \sim \chi^2_{2m-2}$. 
\end{document}